...
public class Log4j2Appender extends AbstractAppender {
  ...
  private static final boolean ENABLED_LOGGING = Objects.equals(
      AqaConfigLoader.getProperty("aqa-trace.logging.enabled", "false"), "true");
  private static final String[] LOGS_ONLY_FROM_PACKAGES = AqaConfigLoader.getProperty(
      "aqa-trace.logging.only-from-package", "").split(",");

  private static final Function<LogEvent, TestExecutionLogDto> CONVERTER = (e) -> new TestExecutionLogDto(
      e.getMessage().getFormattedMessage(), e.getLevel().toString(),
  ...
  public void append(LogEvent event) {
    if (ENABLED_LOGGING && AqaConfigLoader.API_ENDPOINT != null && !event.getLoggerName()
        .startsWith("com.aqanetics.agent.testng")
        && ExecutionEntities.inProgressTestExecutionId != null
        && (LOGS_ONLY_FROM_PACKAGES.length == 0 || Arrays.stream(LOGS_ONLY_FROM_PACKAGES)
